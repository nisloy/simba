...
const handleAuth = () => {
  console.log(`${activeTab.value} attempt:`, email.value)
  emit('close')
}
</script>
          ...
          <div class="md:w-1/2 p-10 md:p-16 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl flex flex-col justify-center relative">
            ...
            <div class="space-y-10 w-full max-w-sm mx-auto">
              ...
              <div class="flex p-1.5 bg-stone-100 dark:bg-zinc-800 rounded-[1.2rem] relative w-full">
                <div 
                  class="absolute inset-y-1.5 w-[calc(50%-6px)] bg-white dark:bg-zinc-700 rounded-xl shadow-md transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1)"
                  :class="activeTab === 'login' ? 'left-1.5' : 'left-[50%]'"
                ></div>
                <button @click="activeTab = 'login'" class="flex-1 relative z-10 py-3 text-sm font-black uppercase tracking-widest transition-colors" :class="activeTab === 'login' ? 'text-orange-600' : 'text-stone-400'">Login</button>
                <button @click="activeTab = 'signup'" class="flex-1 relative z-10 py-3 text-sm font-black uppercase tracking-widest transition-colors" :class="activeTab === 'signup' ? 'text-orange-600' : 'text-stone-400'">Sign Up</button>
              </div>
              ...
              <div class="space-y-6">
                <div class="relative">
                  <input 
                    ...
                    type="text" 
                    id="email"
                    class="peer w-full bg-stone-50 dark:bg-zinc-800/50 border-none rounded-2xl py-5 px-8 text-base font-bold focus:ring-4 focus:ring-orange-500/10 transition-all outline-none placeholder-transparent"
                    placeholder="Email"
                  >
                ...
                >
                  {{ activeTab === 'login' ? 'Continue Shopping' : 'Create My Account' }}
                </button>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-3">
                  <button class="w-full py-4 border-2 border-stone-100 dark:border-zinc-800 hover:bg-stone-50 dark:hover:bg-zinc-800 text-stone-900 dark:text-stone-100 font-black rounded-2xl transition-all active:scale-95 text-[11px] uppercase tracking-widest flex items-center justify-center gap-4">
                  ...
                  <button class="w-full py-4 bg-black text-white hover:bg-zinc-800 font-black rounded-2xl transition-all active:scale-95 text-[11px] uppercase tracking-widest flex items-center justify-center gap-4">
